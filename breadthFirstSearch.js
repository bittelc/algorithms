...
var doBFS = function(graph, source) {
    ...
    while (!queue.isEmpty()) {
        ...
        var nextOut = queue.dequeue();
        
        for (var v = 0; v < graph[nextOut].length; v++) {
            
            var connectedVertex = graph[nextOut][v];
            if (bfsInfo[connectedVertex].distance === null) {
                bfsInfo[connectedVertex].distance = bfsInfo[nextOut].distance + 1;
                bfsInfo[connectedVertex].predecessor = nextOut;
